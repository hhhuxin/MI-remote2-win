...
from tkinter import BOTH, END, LEFT, RIGHT, X, Y, Canvas, Entry, Frame, Label, StringVar, Tk, Button
...
class RemotePrototypeApp:
    ...
    def _build_shell(self):
        ...
        Label(self.sidebar, text="小米遥控器 2 PC", bg=SIDEBAR, fg=TEXT, font=("Segoe UI", 16, "bold"), anchor="w").pack(fill=X, padx=22, pady=(28, 4))
        Label(self.sidebar, text="Windows 11 控制中心", bg=SIDEBAR, fg=MUTED, font=("Segoe UI", 9), anchor="w").pack(fill=X, padx=23, pady=(0, 28))
        for key, label in (("remote", "遥控器"), ("mapping", "按键映射"), ("settings", "设置")):
            button = Button(self.sidebar, text="  " + label, command=lambda k=key: self.show_page(k), relief="flat", bd=0, anchor="w", bg=SIDEBAR, activebackground="#dcdce1", activeforeground=TEXT, fg=MUTED, font=("Segoe UI", 11), padx=12, pady=10, cursor="hand2")
            button.pack(fill=X, padx=12, pady=2)
            self.nav_buttons[key] = button
        status = Frame(self.sidebar, bg=SIDEBAR)
        status.pack(side="bottom", fill=X, padx=20, pady=24)
        Label(status, text="●", fg=GREEN, bg=SIDEBAR, font=("Segoe UI", 13)).pack(side=LEFT)
        Label(status, textvariable=self.status, fg=TEXT, bg=SIDEBAR, font=("Segoe UI", 10)).pack(side=LEFT, padx=7)
        Label(status, text="小米遥控器 2 PC", fg=MUTED, bg=SIDEBAR, font=("Segoe UI", 9)).pack(side=LEFT, padx=8)
    ...
    def _header(self, title, subtitle):
        ...
        Label(header, text=title, bg=BG, fg=TEXT, font=("Segoe UI", 25, "bold"), anchor="w").pack(fill=X)
        Label(header, text=subtitle, bg=BG, fg=MUTED, font=("Segoe UI", 10), anchor="w").pack(fill=X, pady=(8, 0))
    ...
    def _remote_page(self):
        ...
        Label(visual, text="设备预览", bg=CARD, fg=MUTED, font=("Segoe UI", 10), anchor="w").pack(fill=X, padx=24, pady=(22, 0))
        ...
        Label(details, text="当前按键", bg=CARD, fg=MUTED, font=("Segoe UI", 10), anchor="w").pack(fill=X, padx=24, pady=(28, 6))
        Label(details, textvariable=self.current_button, bg=CARD, fg=TEXT, font=("Segoe UI", 18, "bold"), anchor="w", wraplength=210).pack(fill=X, padx=24)
        Frame(details, bg=LINE, height=1).pack(fill=X, padx=24, pady=25)
        Label(details, text="连接状态", bg=CARD, fg=MUTED, font=("Segoe UI", 10), anchor="w").pack(fill=X, padx=24, pady=(0, 6))
        Label(details, text="● 已连接", bg=CARD, fg=GREEN, font=("Segoe UI", 12), anchor="w").pack(fill=X, padx=24)
        Label(details, text="按键监听已准备好\n第一阶段使用模拟交互", bg=CARD, fg=MUTED, font=("Segoe UI", 9), justify="left", anchor="w").pack(fill=X, padx=24, pady=(14, 0))
    ...
    def _mapping_page(self):
        ...
        Label(top, text="原始功能", bg=CARD, fg=MUTED, font=("Segoe UI", 9, "bold"), width=25, anchor="w").pack(side=LEFT)
        Label(top, text="自定义功能", bg=CARD, fg=MUTED, font=("Segoe UI", 9, "bold"), width=38, anchor="w").pack(side=LEFT)
        Label(top, text="恢复", bg=CARD, fg=MUTED, font=("Segoe UI", 9, "bold"), anchor="w").pack(side=RIGHT, padx=8)
        Frame(card, bg=LINE, height=1).pack(fill=X, padx=26)
        rows = Frame(card, bg=CARD); rows.pack(fill=BOTH, expand=True, padx=26, pady=(6, 12))
        self.mapping_vars = {}
        for button in REMOTE_BUTTONS:
            row = Frame(rows, bg=CARD, height=42); row.pack(fill=X, pady=2); row.pack_propagate(False)
            Label(row, text=self.bindings[button]["original"], bg=CARD, fg=TEXT, font=("Segoe UI", 10), width=25, anchor="w").pack(side=LEFT)
            var = StringVar(value=self.bindings[button]["mapping"]); self.mapping_vars[button] = var
            entry = Entry(row, textvariable=var, bg="#fafafa", fg=TEXT, insertbackground=BLUE, relief="flat", highlightthickness=1, highlightbackground=LINE, highlightcolor=BLUE, font=("Segoe UI", 10))
            ...
            Button(row, text="恢复默认", command=lambda b=button: self._reset_mapping(b), bg="#f2f2f5", activebackground="#e3e3e8", fg=MUTED, relief="flat", bd=0, padx=10, pady=5, cursor="hand2").pack(side=RIGHT)
        bottom = Frame(card, bg=CARD); bottom.pack(fill=X, padx=26, pady=(4, 20))
        Label(bottom, text="修改只影响右侧自定义功能，左侧原始功能不会变化。", bg=CARD, fg=MUTED, font=("Segoe UI", 9), anchor="w").pack(side=LEFT)
        Button(bottom, text="恢复全部默认", command=self._reset_all, bg=BLUE, fg="white", activebackground="#006de0", relief="flat", bd=0, padx=14, pady=7, cursor="hand2").pack(side=RIGHT)
